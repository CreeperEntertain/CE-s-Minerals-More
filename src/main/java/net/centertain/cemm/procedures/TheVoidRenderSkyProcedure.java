...
public class TheVoidRenderSkyProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != TheVoidRenderSkyProcedure.amount || seed != TheVoidRenderSkyProcedure.seed) {
			TheVoidRenderSkyProcedure.amount = amount;
			TheVoidRenderSkyProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != TheVoidRenderSkyProcedure.effects) {
				TheVoidRenderSkyProcedure.effects = effects;
				TheVoidRenderSkyProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			TheVoidRenderSkyProcedure.customSky.add(new Consumer<Object[]>() {
				@Override
