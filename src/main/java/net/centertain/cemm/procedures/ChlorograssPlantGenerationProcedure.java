...
public class ChlorograssPlantGenerationProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if ((world.getBlockState(new BlockPos(x, y + 1, z))).getBlock() == Blocks.AIR) {
			Rand = Math.random();
			if (Rand <= 0.01) {
				world.setBlock(new BlockPos(x, y + 1, z), CemmModBlocks.CHLOROWOOD_SAPLING.get().defaultBlockState(), 3);
			} else if (Rand <= 0.05) {
				...
				if ((world.getBlockState(new BlockPos(x, y + 2, z))).getBlock() == Blocks.AIR) {
					...
					world.setBlock(new BlockPos(x, y + 1, z), CemmModBlocks.CHLOROGRASS.get().defaultBlockState(), 3);
				}
			} else if (Rand <= 0.4) {
				world.setBlock(new BlockPos(x, y + 1, z), CemmModBlocks.CHLOROGRASS.get().defaultBlockState(), 3);
			} else {
