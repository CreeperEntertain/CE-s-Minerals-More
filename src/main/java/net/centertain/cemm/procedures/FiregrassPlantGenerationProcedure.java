...
public class FiregrassPlantGenerationProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == Blocks.AIR) {
			Rand = Math.random();
			if (Rand <= 0.05) {
				world.setBlock(BlockPos.containing(x, y + 1, z), CemmModBlocks.FIREFRUIT_SHRUB.get().defaultBlockState(), 3);
			} else if (Rand <= 0.2) {
				if ((world.getBlockState(BlockPos.containing(x, y + 2, z))).getBlock() == Blocks.AIR) {
					...
					world.setBlock(BlockPos.containing(x, y + 1, z), CemmModBlocks.FIREGRASS.get().defaultBlockState(), 3);
				}
			} else if (Rand <= 0.7) {
				world.setBlock(BlockPos.containing(x, y + 1, z), CemmModBlocks.FIREGRASS.get().defaultBlockState(), 3);
			}
