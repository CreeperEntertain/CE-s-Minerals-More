...
public class InfoSignPlacementConditionProcedure {
	public static boolean execute(LevelAccessor world, double x, double y, double z, BlockState blockstate) {
		if (world.getBlockState(new BlockPos(x + 1, y, z)).canOcclude() && blockstate == (new Object() {
		...
		if (world.getBlockState(new BlockPos(x, y, z + 1)).canOcclude() && blockstate == (new Object() {
			public BlockState with(BlockState _bs, Direction newValue) {
				Property<?> _prop = _bs.getBlock().getStateDefinition().getProperty("facing");
				if (_prop instanceof DirectionProperty _dp && _dp.getPossibleValues().contains(newValue))
					return _bs.setValue(_dp, newValue);
