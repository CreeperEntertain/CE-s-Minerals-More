...
public class IcaronRenderSkyProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != IcaronRenderSkyProcedure.amount || seed != IcaronRenderSkyProcedure.seed) {
			IcaronRenderSkyProcedure.amount = amount;
			IcaronRenderSkyProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!pattern.matcher(effects.getName()).find())
				return;
			if (effects != IcaronRenderSkyProcedure.effects) {
				IcaronRenderSkyProcedure.effects = effects;
				IcaronRenderSkyProcedure.customSky = (List<Consumer<Object[]>>) effects.getField("customSky").get(null);
			}
			IcaronRenderSkyProcedure.customSky.add(new Consumer<Object[]>() {
				@Override
	...
	private static void execute(@Nullable Event event, ResourceKey<Level> dimension) {
		if (dimension == null)
			return;
		if (dimension == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("cemm:icaron"))) {
			renderSky(false, true, true, true, false, false);
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			RenderSystem.setShaderTexture(0, new ResourceLocation(("cemm" + ":textures/" + "icaron_skybox" + ".png")));
