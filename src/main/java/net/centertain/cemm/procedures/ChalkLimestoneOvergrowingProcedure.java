...
public class ChalkLimestoneOvergrowingProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		String registryName = "";
		if (Math.random() <= 0.2) {
			if ((ForgeRegistries.BLOCKS.getKey((world.getBlockState(BlockPos.containing(x, y, z))).getBlock()).toString()).startsWith("minecraft:")
					|| (ForgeRegistries.BLOCKS.getKey((world.getBlockState(BlockPos.containing(x, y, z))).getBlock()).toString()).startsWith("cemm:")) {
				if (!(ForgeRegistries.BLOCKS.getValue(new ResourceLocation(
						(("cemm:grassed_" + (ForgeRegistries.BLOCKS.getKey((world.getBlockState(BlockPos.containing(x, y, z))).getBlock()).toString()).replace("cemm:", ""))).toLowerCase(java.util.Locale.ENGLISH))) == Blocks.AIR)) {
				...
				} else if (!(ForgeRegistries.BLOCKS.getValue(new ResourceLocation(
						(("cemm:grassed_" + (ForgeRegistries.BLOCKS.getKey((world.getBlockState(BlockPos.containing(x, y, z))).getBlock()).toString()).replace("minecraft:", ""))).toLowerCase(java.util.Locale.ENGLISH))) == Blocks.AIR)) {
