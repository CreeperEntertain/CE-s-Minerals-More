...
public class ProceduralSmolderbarkTrunkProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		for (int index0 = 0; index0 < (int) RAND; index0++) {
			...
			world.setBlock(BlockPos.containing(x, Y, z), CemmModBlocks.SMOLDERBARK_LOG.get().defaultBlockState(), 3);
			Y = Y + 1;
		}
		if ((world.getBlockState(BlockPos.containing(x, Y, z))).is(BlockTags.create(new ResourceLocation("minecraft:logs")))) {
			SmolderbarkCrownProcedure.execute(world, x, Y, z);
		} else {
			SmolderbarkCrownProcedure.execute(world, x, (Y - 1), z);
		}
	}
}
