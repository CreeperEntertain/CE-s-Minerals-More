...
public class IcaronRenderFogProcedure {
	...
	private static void execute(@Nullable Event event, ResourceKey<Level> dimension) {
		...
		if (dimension == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("cemm:icaron"))) {
			setDistance((float) (Minecraft.getInstance().gameRenderer.getRenderDistance() / 4), (float) Minecraft.getInstance().gameRenderer.getRenderDistance());
