...
public class WoodStrippingCharredProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!entity.isShiftKeyDown()) {
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == CemmModBlocks.CHARRED_WOOD.get()) {
				if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() instanceof AxeItem) {
						...
						_entity.swing(InteractionHand.MAIN_HAND, true);
					{
						...
						BlockState _bs = CemmModBlocks.STRIPPED_CHARRED_WOOD.get().defaultBlockState();
					...
					if (!(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
						...
						for (int index0 = 0; index0 < Mth.nextInt(RandomSource.create(), 1, 2); index0++) {
							if (world instanceof ServerLevel _level) {
								ItemEntity entityToSpawn = new ItemEntity(_level, (x + Math.random()), (y + Math.random()), (z + Math.random()), new ItemStack(CemmModItems.CHARCOAL_PIECES.get()));
