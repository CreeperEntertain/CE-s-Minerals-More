	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (TimeDisplayDisplayConditionProcedure.execute(entity)) {
			if (OreDisplayVisibilityProcedure.execute(entity))
				event.getGuiGraphics().drawString(Minecraft.getInstance().font, Component.translatable("gui.cemm.ore_display.label_time"), w / 2 + -207, h / 2 + 97, -16737793, false);
			if (OreDisplayVisibilityProcedure.execute(entity))
				event.getGuiGraphics().drawString(Minecraft.getInstance().font,

						OreDisplayBarProcedure.execute(world, x, y, z, entity), w / 2 + -167, h / 2 + 97, -1, false);
		}
	}
}
