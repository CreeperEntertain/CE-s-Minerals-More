	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (TimeDisplayDisplayConditionProcedure.execute(entity)) {
			if (OxygenDisplayConditionProcedure.execute(entity))
				event.getGuiGraphics().drawString(Minecraft.getInstance().font, Component.translatable("gui.cemm.oxygen_display.label_oxygen_left"), w / 2 + -207, h / 2 + 86, -52429, false);
			if (OxygenDisplayConditionProcedure.execute(entity))
				event.getGuiGraphics().drawString(Minecraft.getInstance().font,

						GetOxygenPercentageProcedure.execute(entity), w / 2 + -167, h / 2 + 86, -1, false);
		}
	}
}
