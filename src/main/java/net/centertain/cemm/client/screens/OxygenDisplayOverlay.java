	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (TimeDisplayDisplayConditionProcedure.execute(entity)) {
			if (OxygenDisplayConditionProcedure.execute(entity))
				event.getGuiGraphics().drawString(Minecraft.getInstance().font, Component.translatable("gui.cemm.oxygen_display.label_oxygen_left"), posX + -207, posY + 86, -52429, false);
			if (OxygenDisplayConditionProcedure.execute(entity))
				event.getGuiGraphics().drawString(Minecraft.getInstance().font,

						GetOxygenPercentageProcedure.execute(entity), posX + -167, posY + 86, -1, false);
		}
	}
}
