...
public class TimeDisplayOverlay {
	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (TimeDisplayDisplayConditionProcedure.execute(entity)) {
			event.getGuiGraphics().drawString(Minecraft.getInstance().font, Component.translatable("gui.cemm.time_display.label_time"), w / 2 + -207, h / 2 + 108, -103, false);
			event.getGuiGraphics().drawString(Minecraft.getInstance().font,

					TimeDisplayValueProcedure.execute(world), w / 2 + -167, h / 2 + 108, -1, false);
		}
	}
}
