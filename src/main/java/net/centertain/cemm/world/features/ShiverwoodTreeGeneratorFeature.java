...
import net.centertain.cemm.procedures.ProceduralShiverwoodTrunkProcedure;
import net.centertain.cemm.init.CemmModBlocks;

import java.util.Set;
import java.util.List;

public class ShiverwoodTreeGeneratorFeature extends Feature<NoneFeatureConfiguration> {
	...
	public ShiverwoodTreeGeneratorFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(CemmModBlocks.VIBRATING_SERANDITE.get(), CemmModBlocks.VIBRATING_MOSS.get());
	}
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo,
					...
					ProceduralShiverwoodTrunkProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
